...
<main class="size-fit flex items-center justify-center flex-col select-none {pause ? "animate-pulse" : ""} hover:scale-110 transition-all duration-300 cursor-pointer drop-shadow-xl" on:click={() => pause = !pause}>
    {#each screen as row}
